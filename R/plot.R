...
CorrPlotTotal <- function(data1 = data1,data2 = data2,type1=NULL,type2=NULL,
                          method="spearman",topn1=50,topn2=50,cutoff_cor=0.6,
                          cutoff_p=0.05) {
  if (identical(colnames(data1),colnames(data2))) {
    ...
    plot_heatmap_data = heatmap_data[c(1:ifelse(nrow(data1)>50,50,nrow(data1))),
                                     c(1:ifelse(nrow(data2)>50,50,nrow(data2)))]
    plot_p_data = cor_data_p[c(1:ifelse(nrow(data1)>50,50,nrow(data1))),
                             c(1:ifelse(nrow(data2)>50,50,nrow(data2)))]
    ...
    p = pheatmap::pheatmap(plot_heatmap_data,
                ...
                main="")
    ggplot2::ggsave(filename = "相关性热图.png",plot = p,
                    width = 8,height = 8.8,dpi = 900)
    ggplot2::ggsave(filename = "相关性热图.pdf",plot = p,
                    width = 8,height = 8.8,dpi = 900)
